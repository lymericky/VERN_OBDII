	...
	implements TelegramListener, TelegramWriter, Runnable
{
	...
	public enum CMD
	{
		...
		SETKEYWORDS("KW", 1, true), //Enable Keywords for ISO9141
		ALLOWLONG("AL", 0, true); // Allow long response for ISO9141
	...
	public class AdaptiveTiming
	{
		 ...
		 *
		 * @return currently learned timeout value [ms]
		 */
	...
	/*
	 * create ELM command string from command id and parameter
	 *
	...
	private void initialize()
	{
		...
		//Display Protocol
		pushCommand(CMD.GETPROT, 0);
		log.info("Display Protocol");

		/*
		* Enable Keywords for protocol 9141-2
		* */
		if (preferredProtocol.equals(PROT.ELM_PROT_9141_2)) {
			pushCommand(CMD.SETKEYWORDS, 1);
			log.info("Keywords Enabled");
			//pushCommand(CMD.ALLOWLONG, 0);
			//log.info("Allow Long");
			//pushCommand(CMD.SETHEADER, 1);
			//log.info("Headers Enabled");
			//pushCommand(CMD.GETPROT, 0);
			//log.info("Display Protocol");
	...
	public synchronized int handleTelegram(char[] buffer)
	{
		...
		switch (getResponseId(bufferStr))
		{
			...
			default:
				...
				{
					case UNDEFINED:
						log.info("ECU Status : UNDEFINED");
						break;
					case INITIALIZING:
						log.info("ECU Status : INITIALIZING");
						break;
					case INITIALIZED:
						log.info("ECU Status : INITIALIZED");
						break;
					case ECU_DETECT:
					{
					...
					case ECU_DETECTED:
						log.info("ECU Status : DETECTED");
						break;
					case CONNECTING:
						log.info("ECU Status : CONNECTING");
						break;
					case CONNECTED:
						log.info("ECU Status : CONNECTED");
						break;
					case NODATA:
						log.info("ECU Status : NO DATA");
						break;
					case STOPPED:
						log.info("ECU Status : STOPPED");
						break;
					case DISCONNECTED:
						log.info("ECU Status : DISCONNECTED");
						break;
					case BUSERROR:
						log.info("ECU Status : BUS ERROR");
						break;
					case DATAERROR:
						log.info("ECU Status : DATA ERROR");
						break;
					case RXERROR:
						log.info("ECU Status : RX ERROR");
						break;
					case ERROR:
						log.info("ECU Status : ERROR");
						break;
					default:
						break;
				}
				...
				// .. or a ISO multi line response with format SVC PID MSG ID DATA...
			...
			try
			{
				...
				while (runDemo)
				{
					switch (service)
					{
						...
						case OBD_SVC_NONE:
							// just keep quiet until someone requests something
							break;
